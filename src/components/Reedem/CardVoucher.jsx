  ...
  return (
    <>
            ...
            {voucher.FoodBeverage.map((item, index) => {
              return (
                <div className=" my-2 cursor-pointer z-0" key={index}>
                  <div
                    className="card rounded-xl h-72 w-56 m-8   p-2  shadow bg-white"
                    onClick={() =>
                      getVoucher(
                        item.image,
                        item.voucher,
                  ...
                  >
                    <img src={item.image} alt="Voucher Partner" className=" rounded-t-xl h-40" />
                    <div className="m-2">
                      <h3 className=" text-lg   ">{item.voucher}</h3>
                      <h1 className=" text-sm absolute bottom-4 right-3">
                        <FontAwesomeIcon
                          icon={faStar}
      ...
      {modal === true ? (
        <Modal
          image={tempVoucher[1]}
          voucher={tempVoucher[2]}
          date={tempVoucher[3]}
          points={tempVoucher[4]}
          hide={() => setModal(false)}
        />
      ) : (
        ""
